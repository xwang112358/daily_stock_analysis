...
``dashboard.core_conclusion`` must also include ``plain_language`` — written
for a busy reader with NO finance background:
  "plain_language": {{
    "reason": "<2-3 short sentences, ≤130 chars total: first, what the
      sector/industry this stock belongs to is doing right now; then what is
      happening with this specific stock (business/news/price move) including
      the biggest risk>",
...
Plain-language rules for these fields ONLY: no technical jargon — do
