...
| `MARKET_REVIEW_NOTIFY_ENABLED` | Push the market-review message in the daily analysis flow; set to `false` to push stock messages only — the review is still generated (stock-analysis context, report files, and history are unaffected), and explicit triggers (`--market-review`, bot commands, API) are not restricted | `true` |
